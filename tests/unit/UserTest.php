...
class UserTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->test_user_id = wp_create_user(
            $this->test_username . '_' . time(),
            $this->test_password,
            $this->test_email . '_' . time()
        );
    ...
    public function test_user_authentication_with_nonexistent_username()
    {
        ...
        $authenticated_user = wp_authenticate('nonexistent_user_' . time(), 'some_password');
        
        // Verify authentication failed
        $this->assertInstanceOf(\WP_Error::class, $authenticated_user, 'Authentication should fail with non-existent username');
        $this->assertEquals('invalid_username', $authenticated_user->get_error_code(), 'Error code should be invalid_username');
    }
    ...
    {
        $user = get_user_by('login', $this->test_username . '_' . time());
        
        // Note: This might fail if username doesn't match exactly
        // So we'll test with the created user's actual username
    ...
    public function test_user_deletion()
    {
        ...
        $temp_user_id = wp_create_user('temp_user_' . time(), 'temp_pass', 'temp' . time() . '@example.com');
        
        if (!is_wp_error($temp_user_id)) {
            // Delete the user
            $deleted = wp_delete_user($temp_user_id);
